...
public class REV_Max extends SubsystemBase {

  /*Motor and Spark Identification Constants */
    public SparkMax REV_motor;
    public SparkClosedLoopController Rev_Max_controller;
    public SparkMaxConfig Rev_Max_config;
    public RelativeEncoder Rev_Max_encoder;
  ...
  public REV_Max(
      ...
      boolean Idle_Brake, 
      boolean Inverted,
      double[] MotionProfile//Holds motor PID, FF, and motion Constrants
    ...
    /*Set value to import or set values ot defualt if null */
      ...
      velocity = MotionProfile[24];
      accleration = MotionProfile[25];
      allowedError = MotionProfile[26];
      this.MotionProfile = MotionProfile;
    
     this.Motor_int_values = Motor_int_values;
      if (Idle_Brake ==true){RunningIdle=IdleMode.kBrake;}else{RunningIdle=IdleMode.kCoast;}

    /*Identifies Motor and encoder*/
        REV_motor = new SparkMax(Motor_int_values[0], MotorType.kBrushless);
        Rev_Max_controller = REV_motor.getClosedLoopController();
        Rev_Max_encoder = REV_motor.getEncoder();
        Rev_Max_config = new SparkMaxConfig();
        ...
        .idleMode(RunningIdle);
          ...
          Rev_Max_config.closedLoop
          // Set gains for position control in slot 0
          .p(MotionProfile[0],ClosedLoopSlot.kSlot0)
          .i(MotionProfile[1],ClosedLoopSlot.kSlot0)
          .d(MotionProfile[2],ClosedLoopSlot.kSlot0)
          .iZone(MotionProfile[4],ClosedLoopSlot.kSlot0)
          .iMaxAccum(MotionProfile[5],ClosedLoopSlot.kSlot0)
          .outputRange(MotionProfile[6],MotionProfile[7],ClosedLoopSlot.kSlot0)
                           
          // Set gains for velocity control in slot 1
          .p(MotionProfile[8],ClosedLoopSlot.kSlot1)
          .i(MotionProfile[9],ClosedLoopSlot.kSlot1)
          .d(MotionProfile[10],ClosedLoopSlot.kSlot1)
          .velocityFF(MotionProfile[11],ClosedLoopSlot.kSlot1)
          .iZone(MotionProfile[12],ClosedLoopSlot.kSlot1)
          .iMaxAccum(MotionProfile[13],ClosedLoopSlot.kSlot1)
          .outputRange(MotionProfile[14],MotionProfile[15],ClosedLoopSlot.kSlot1)

          // Set gains for velocity control in slot 2
          .pidf(MotionProfile[16],MotionProfile[17],MotionProfile[18],MotionProfile[19],ClosedLoopSlot.kSlot2)
          .iZone(MotionProfile[20],ClosedLoopSlot.kSlot2)
          .iMaxAccum(MotionProfile[21],ClosedLoopSlot.kSlot2)
          .outputRange(MotionProfile[22],MotionProfile[23],ClosedLoopSlot.kSlot2);

          // Slot 3 is avaliable for more control
                    
        /*Configures motion limits*/
          Rev_Max_config.closedLoop.maxMotion
          .maxVelocity(velocity,ClosedLoopSlot.kSlot0)
          .maxAcceleration(accleration,ClosedLoopSlot.kSlot0)
          .allowedClosedLoopError(allowedError,ClosedLoopSlot.kSlot0)

          .maxVelocity(velocity,ClosedLoopSlot.kSlot1)
          .maxAcceleration(accleration,ClosedLoopSlot.kSlot1)
          .allowedClosedLoopError(allowedError,ClosedLoopSlot.kSlot1);
          
        
          /*Set update frequency */
          Rev_Max_config.signals.primaryEncoderPositionPeriodMs(10);
                 
          /*Applies configuration */
          REV_motor.configure(Rev_Max_config, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
  ...
  /*Getter-Get values from the motors */
    public SparkMaxConfig getConfig(){//Get the config to pass thought to a follower motor
      return Rev_Max_config;
    }

    public SparkMax getMotor(){//Get the motor to pass thought to a follower motor
      return REV_motor;
    }
    ...
    public Rotation2d getMotorPosition(){//Get the position of the motor and translates it to Rotation2D
      return Rotation2d.fromRotations(Rev_Max_encoder.getPosition());
    }

    public Rotation2d getMotorVelocity_RPM(){//Get the velocity of the motor and translates it to Rotation2D per minute
      return Rotation2d.fromRotations(Rev_Max_encoder.getVelocity());
    }
  ...
  /*Setter-Used to set the values of motors*/
    public void setCurrentMechanismPosition(double position_rotations){//Set current positon of the encoder
      Rev_Max_encoder.setPosition(position_rotations);
    }

    public void setTargetMotorPosition(double setPoint_rotations){//Sets the target position for the motor
      Rev_Max_controller.setReference(setPoint_rotations, ControlType.kMAXMotionPositionControl, ClosedLoopSlot.kSlot0);
    }

    public void setTargetMotorVelocity(double setPoint_rotations_per_minute){//Sets the target velocity for the motor
      Rev_Max_controller.setReference(setPoint_rotations_per_minute, ControlType.kMAXMotionVelocityControl,ClosedLoopSlot.kSlot1);
    }

    public void setTargetMotorVoltage(double setPoint_volts){//Sets the target voltage for the motor
      Rev_Max_controller.setReference(setPoint_volts, ControlType.kVoltage, ClosedLoopSlot.kSlot2);
    }
    ...
    public void setTargetMechanismPosition(double setPoint_rotations){//Sets the target positon for the motor after the grear ratio
      Rev_Max_controller.setReference(setPoint_rotations/GearRatio, ControlType.kMAXMotionPositionControl, ClosedLoopSlot.kSlot0);
    }
    ...
    public void setTargetMechanismVelocity(double setPoint_rotations_per_minute){//Sets the target velocity for the motor after the grear ratio
      Rev_Max_controller.setReference(setPoint_rotations_per_minute/GearRatio, ControlType.kMAXMotionVelocityControl, ClosedLoopSlot.kSlot1);
    }
    ...
    public void setTargetMechanismTipSpeed(double meter_per_second){//Sets the target velocity for the motor after the grear ratio and wheel circumfernce
      setTargetMechanismVelocity(meter_per_second/(2*Math.PI*WheelRadius)/GearRatio*60);
    }


    public void setIdleState(boolean is_Break){//Sets the motor to an Idle state should be called once
    ...
    public void setTunerConstants(double new_MotionProfile[], int SlotNumber){//Sets the motor tunerConstants should be called once 
      ...
      //Make new config
      SparkMaxConfig config = new SparkMaxConfig();
      double set_MotionProfile[]=MotionProfile;
      ...
      if(SlotNumber==0){//Slot choosen is zero
        ...
        if (new_MotionProfile[i] != (Double) null){set_MotionProfile[i]=new_MotionProfile[i];}//If values have changed changes them
          }
          config.closedLoop//Define new config values
          .pidf(MotionProfile[0],MotionProfile[1],MotionProfile[2],MotionProfile[3],ClosedLoopSlot.kSlot0)
          .iZone(MotionProfile[4],ClosedLoopSlot.kSlot0)
          .iMaxAccum(MotionProfile[5],ClosedLoopSlot.kSlot0)
          .outputRange(MotionProfile[6],MotionProfile[7],ClosedLoopSlot.kSlot0);}
      ...
      config.closedLoop //Define new config values
      .pidf(MotionProfile[8],MotionProfile[9],MotionProfile[10],MotionProfile[11],ClosedLoopSlot.kSlot1)
      .iZone(MotionProfile[12],ClosedLoopSlot.kSlot1)
      .iMaxAccum(MotionProfile[13],ClosedLoopSlot.kSlot1)
      .outputRange(MotionProfile[14],MotionProfile[15],ClosedLoopSlot.kSlot1);}
     ...
     private final SysIdRoutine m_sysIdRoutineTranslation = new SysIdRoutine(
        ...
        new SysIdRoutine.Mechanism(
            output -> 
                ...
                Rev_Max_controller.setReference(output.in(Volts),ControlType.kVoltage),
            null,
            this
        )//      Rev_Max_controller.setReference(setPoint_volts, ControlType.kVoltage, ClosedLoopSlot.kSlot2);
